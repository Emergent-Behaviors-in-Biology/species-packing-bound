...
Pool_num=28
file_name='Community_'+C_type+'_'+B_type +'_'+dynamics+'_'+Simulation_type+'_log_map.csv'
...
for S in [100]:
	parameters['S'] =S*5;
	parameters['M'] =S
	parameters['sample_size']=int(100*1/S);
	#for mu in np.append(0,np.logspace(-3.0, 2., num=10)):
	#for mu in [0, 0.6, 1.0, 3.0, 5.0, 8.0, 10.0]:
	mu=1.0
	for K in np.logspace(-1,2., num=100):  
		parameters['K'] =K
		for epsilon in np.logspace(-2,2., num=100):  
			#for epsilon in np.linspace(0.0, 2.0, num=201): 
				jobs.append([parameters['sample_size'],parameters['S'],parameters['M'],parameters['K'],parameters['sigma_K'], parameters['mu'], parameters['sigma_c'],parameters['m'],parameters['sigma_m'],parameters['loop_size'],parameters['t0'],parameters['t1'],parameters['Nt']  ,epsilon, mu, 0])
